...
-(void)startPinging {
    if (self.isReady && !self.isPinging) {
        ...
        NSThread *listenThread = [[NSThread alloc] initWithTarget:self selector:@selector(listenLoop:) object:self.delegate];
        listenThread.name = @"listenThread";
        
        //set up loop that sends packets on a new thread (sendThread)
        NSThread *sendThread = [[NSThread alloc] initWithTarget:self selector:@selector(sendLoop:) object:self.delegate];
